...
for eps in epsilons:
    ...
    q_player1 = DeepQPlayer(epsilon=eps, target_update=500, batch_size=64, log_every=250,
                            policy_net=policy_net, target_net=target_net, memory=memory, wandb_name=f"q16_no_swap_no_optimize_eps{eps}")
    q_player2 = DeepQPlayer(epsilon=eps, target_update=500, batch_size=64, log=False,
                            policy_net=policy_net, target_net=target_net, memory=memory, do_optimize=False)
